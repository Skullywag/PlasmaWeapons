...
namespace PlasmaWeapons
{
    public class Projectile_PlasmaGrenade : Projectile
    {
        ...
        protected virtual void Explode()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ThrowSmokeGreen(Position.ToVector3Shifted() + Gen.RandomHorizontalVector(this.def.projectile.explosionRadius * 0.7f), this.def.projectile.explosionRadius * 0.6f);
                ThrowMicroSparksGreen(Position.ToVector3Shifted() + Gen.RandomHorizontalVector(this.def.projectile.explosionRadius * 0.7f));
            }
        }
